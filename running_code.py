...
result_2 = mashup.getEntityById("VIAF:100219162")
...
#result_10 = mashup.getActivitiesStartedAfter("2023-05-08")
#result_11 = mashup.getActivitiesEndedBefore("2023-04-07")
...
#print(f"All the authors: {result_1} the number {len(result_1)}")
...
#print(f"All the Ids of the Cultural Heritage Objects {result_7}, number is {len(result_7)}")
#print(f"All the activities by responsible person: {result_8}")
#print(f"Activities done using the tool: {result_9}")
#print(f"First activity started after the date: {[i.getStartDate() for i in result_10]}, the number{len(result_10)}")
#print(f"Activities ended before the date: {[i.getTools() for i in result_11]}, the number {len(result_11)}")
#print(f"Acquisitions done by the technique: {result_12}")
#print(f"Objects handled by the responsible person: {result_13}")
#print(f"A list of object ids handled by responsible institution: {[i.getTitle() for i in result_14]}")
